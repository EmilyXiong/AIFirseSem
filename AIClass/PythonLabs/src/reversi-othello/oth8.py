...
def minimax(pzl, depth, player, isMaxPlayer, alpha, beta):
    ...
    if isMaxPlayer :
        ...
        if not nextMoves:  
            newPzl = pzl
            bestVal = minimax(newPzl, depth+1, player, False, alpha, beta)
        for pos in nextMoves :
            newPzl = flip(pzl, player, pos)
            value = minimax(newPzl, depth+1, player, False, alpha, beta)
        ...
        if not nextMoves:
            newPzl = pzl
            bestVal =  minimax(newPzl, depth+1,player, True, alpha, beta)
        for pos in nextMoves :
            newPzl = flip(pzl, getOpp(player), pos)
            value = minimax(newPzl, depth+1,player, True, alpha, beta)
...
def findBestMove(pzl, player):
    ...
    for pos in nextMoves :
        newPzl = flip(pzl, player, pos)
        value = minimax(newPzl, 0, player, False, -INFINITE, INFINITE)
        if value > bestValue:
            bestValue = value
            bestMove = pos
    return bestMove
