...
def negamax(board, token, levels):  
    if len(legalMoves(board, token))== 0 and len(legalMoves(board, playertable[token])) == 0: 
...
def main():
    ...
    if pzl.count('.')<=8:    
        ...
        print('guaranteed score: {}, sequence of moves in reversed order {}, next step is {}'.format(nm[0], nm[1:], nm[-1]))
    else:
