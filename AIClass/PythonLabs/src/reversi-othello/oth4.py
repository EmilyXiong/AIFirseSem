...
# board is optional
# token is optional and is for human player
...
if isValidBoardArgument(in1):
    ...
    if isValidPlayerArgument(sys.argv[2]):
        if sys.argv[2] == 'O' or sys.argv[2] == 'X': #force next human move
            player = sys.argv[2].lower()
        else:
            player = determinPlayer(pzl)
        player = sys.argv[2]
        startPosIndex = 3
    else:
        player = determinPlayer(pzl)
...
while currentIndex <= maxArgumentIndex:
    ...
    if moves: 
        ...
        if not inBoard(playPos) or playPos not in moves:
            print("A invalid move detected.     " + sys.argv[currentIndex])
            sys.exit()
