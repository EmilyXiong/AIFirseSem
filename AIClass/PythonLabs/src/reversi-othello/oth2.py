...
if len(sys.argv) == 4:
    if isValidBoardArgument(sys.argv[1]) and isValidPlayerArgument(sys.argv[2]) and isValidPosArgument(str(converPosition(sys.argv[3]))):
        pzl = sys.argv[1].lower()
        player = sys.argv[2].lower()
        playPos = converPosition(sys.argv[3])
    else:
        exitGameWithError("Invalid argument(s) !")
elif len(sys.argv) == 3:
    if not isValidPosArgument(str(converPosition(sys.argv[2]))):
        exitGameWithError("Invalid position argument !")
    playPos = converPosition(sys.argv[2])
    ...
    if not isValidPosArgument(str(converPosition(sys.argv[1]))):
        showBoard(pzl)
        print(pzl + " "+ str(pzl.count('x')) + '/'+ str(pzl.count('o'))) 
        sys.exit()   
    playPos = converPosition(sys.argv[1])
else:
    showBoard(pzl)
    print(pzl + " "+ str(pzl.count('x')) + '/'+ str(pzl.count('o')))
    sys.exit()
